...
async function init() {
  ...
  try {
    ...
    document.getElementById('exam-title').textContent = exam.name || 'Bài thi';
    document.getElementById('exam-info').textContent = `${questions.length} câu hỏi • ${session.durationMinutes} phút`;

    renderQuestion();
    startTimer();
  } catch (err) {
    alert('Không tải được bài thi: ' + (err.message || err));
    window.location.href = './index.html';
  }
}
