...
public class AttemptService {
    ...
    public SaveAnswerResponse saveAnswer(String attemptId, String studentId, SaveAnswerRequest req) {
        ...
        if (attempt == null) {
            throw new ApiException(HttpStatus.NOT_FOUND, "404", "Attempt not found");
        }

        String ownerId = (String) attempt.get("studentId");
        if (!Objects.equals(ownerId, studentId)) {
            throw new ApiException(HttpStatus.FORBIDDEN, "403", "You cannot modify this attempt");
        }

        String status = (String) attempt.get("status");
        if (!"in_progress".equals(status)) {
            throw new ApiException(HttpStatus.CONFLICT, "409", "Attempt is already submitted");
        }

        boolean questionExists = Boolean.TRUE.equals(attempt.get("questionExists"));
        if (!questionExists) {
            throw new ApiException(HttpStatus.NOT_FOUND, "404", "Question not found in this exam");
        }

        Map<String, Object> saved = attemptRepository.saveAnswer(attemptId, req.questionId(), req.selectedOptionIndex());
        if (saved == null) {
            throw new ApiException(HttpStatus.INTERNAL_SERVER_ERROR, "400", "Failed to save answer");
        }
    ...
    public void submitAttempt(String attemptId, String studentId) {
        ...
        String ownerId = (String) attempt.get("studentId");
        if (!Objects.equals(ownerId, studentId)) {
            throw new ApiException(HttpStatus.FORBIDDEN, "403", "You cannot submit this attempt");
        }
        if (!"in_progress".equals(attempt.get("status"))) {
            throw new ApiException(HttpStatus.CONFLICT, "409", "Attempt is already submitted");
        }
        if (resultService.existsResultByAttemptId(attemptId)) {
            throw new ApiException(HttpStatus.CONFLICT, "409", "Result already exists for this attempt");
        }
        ...
        if (submittedAt == null) {
            throw new ApiException(HttpStatus.INTERNAL_SERVER_ERROR, "400", "Failed to submit attempt");
        }
        ...
        if (createdResult == null) {
            throw new ApiException(HttpStatus.INTERNAL_SERVER_ERROR, "400", "Failed to create result");
        }
    }
